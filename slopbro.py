...
ENTRY_PAGE = "index-webos26.html"
SHELL_SCRIPT = "autoroot-webos26.sh"
SERVICE_FILES = [
    "package.json",
    "main.js",
]
...
class WebSocket(object):
    ...
    def connect(
        ...
        if secure:
            ...
            sock = raw
        ws = cls(sock)
        ws._handshake(host, port, secure, path)
        return ws

    def _handshake(self, host, port, secure, path="/"):
...
def run(
    ...
    try:
        source_address = (
            (local_ip_override, 0) if local_ip_override is not None else None
        )
        if source_address is not None:
            log("binding TV connection to local IP %s" % local_ip_override)
